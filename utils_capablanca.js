import { Board } from './board_capablanca';
...
export const WHITE_COLUMN_LABEL_SQUARES = [
    'A1',
    'B1',
    'C1',
    'D1',
...
export const BLACK_COLUMN_LABEL_SQUARES = [
    'A6',
    'B7',
    'C8',
    'D9',
...
export const ANNOTATED_WHITE_SQUARES = [
    'A1',
    'A2',
    'A3',
    'A4',
...
export const ANNOTATED_BLACK_SQUARES = [
    'A6',
    'B7',
    'C8',
    'D9',
...
export const fenToBoard = (position) => {
...
export const movesToString = (moves, terminator) => {
    const result = [];
    ...
    const normalizedTerminator = typeof terminator === 'boolean'
        ? terminator
            ? 'resignation'
            : undefined
        : terminator;
...
const moveRegex = /^[A-L^J](10|11|12|[1-9])(-|x)[A-L^J](10|11|12|[1-9])([pPnNqQrRbB])?\$?(=[QqRrBbNn])?$/;
export const stringToMoves = (movesStr) => {
    if (movesStr === '') {
        return [];
    }
    ...
    for (let i = 0; i < moves.length; i++) {
        ...
        if (move in TERMINATION_LABELS) {
            if (i !== moves.length - 1) {
                throw new Error(`Cannot process ${TERMINATION_LABELS[move]} marker when subsequent moves still exist.`);
            }
            break;
        }
        ...
        let toSquare;
        if (isPromotion) {
            promotionPiece = chunks[1].split('=')[1];
        ...
        if (!ALL_SQUARES.includes(toSquare)) {
            throw new Error(`Invalid move: ${move}`);
        }
        if (isCapture) {
...
export const validatePosition = (position) => {
    ...
    for (let i = 0; i < 11; i++) {
        ...
        if ((name === 'A' || name === 'N') && convertedColumn.length !== 6) {
            return null;
        }
        if ((name === 'B' || name === 'M') && convertedColumn.length !== 7) {
            return null;
        }
        if ((name === 'C' || name === 'L') && convertedColumn.length !== 8) {
            return null;
        }
        if ((name === 'D' || name === 'K') && convertedColumn.length !== 9) {
            return null;
        }
        if ((name === 'E' || name === 'I') && convertedColumn.length !== 10) {
            return null;
        }
        if ((name === 'F' || name === 'H') && convertedColumn.length !== 11) {
            return null;
        }
        if (name === 'G' && convertedColumn.length !== 12) {
            return null;
        }
        for (let j = 0; j < convertedColumn.length; j++) {
            const square = `${name}${j + 1}`;
            partialBoard.addPieceFromString(square, convertedColumn[j] === '1' ? null : convertedColumn[j]);
        }
    }
    return partialBoard;
};
...
const replaceNumbers = (string) => {
    return string
        .replaceAll('12', '111111111111') //not sure if this is necessary...
...
//# sourceMappingURL=utils_capablanca.js.map